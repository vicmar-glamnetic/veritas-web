...
/**
 * Editing happens in a dialog, handled by ServiceTable.
 *
 * The inline panel below is the fallback: `?edit=<id>` and `?new=1` are real URLs, so
 * with JavaScript disabled the Edit links still land on a working form. With JavaScript
 * the click is intercepted and the dialog opens instead, and these URLs are never
 * reached.
 */
export default async function ServicesAdminPage({
  ...
  const showFallbackForm = Boolean(editing) || isNew === '1';
    ...
    <div className="space-y-6">
        ...
        <Panel title={editing ? `Editing: ${editing.name}` : 'Add a service'}>
          <form action={saveService} className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="flex flex-wrap items-end gap-5">
              <Checkbox name="isBookableOnline" label="Bookable online" defaultChecked={editing?.isBookableOnline ?? false} />
              <Checkbox name="isListedOnline" label="Shown on prices" defaultChecked={editing?.isListedOnline ?? true} />
      ...
      <ServiceTable
        rows={list.map((s) => ({
          ...
          name: s.name,
          category: s.category,
          priceLabel: formatPhp(s.pricePhp),
          pricePhp: s.pricePhp,
